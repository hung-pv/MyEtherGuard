...
public class BIP39 {
	public static byte[] decode(String mnemonic, String passphrase) throws ValidationException {
		...
		for (i = bits.length / 33 * 32; i < bits.length; ++i) {
			if ((check[(i - bits.length / 33 * 32) / 8] & (1 << (7 - (i % 8)))
					^ (bits[i] ? 1 : 0) << (7 - (i % 8))) != 0) {
				throw new ValidationException("invalid mnemonic - checksum failed");
			}
		}
		try {
			SecretKey seedkey = new SecretKeySpec(StringUtils.getBytes(passphrase, 16), "Blowfish");
	...
	public static byte[] encode(byte[] data, String passphrase) throws ValidationException {
		if (data.length % 8 != 0) {
	...
	private final static String[] english = { "abandon", "ability", "able", "about", "above", "absent", "absorb",
